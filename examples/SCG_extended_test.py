...
def test_training():
    ...
    ordered = supply_chain.add_var("ordered", var_attributes=[
        'product_type'])  # orders stay are dispatched and their existence can be observed (for now)
    ...
    # supply_chain.set_unobservable(simvars = ['in_transit'], token_attrs = {'arrival': ['id']})
    ...
    # Assemble actions
    finished = supply_chain.add_var("finished", var_attributes=['id'])
    supply_chain.add_action([stock_phone_cases, stock_chips], [finished],
                            behavior=lambda x, y: [SimToken({'product_type': 4}, delay=1)],
                            name="produce_phone", reward_function=lambda x, y: 2)
    supply_chain.add_action([stock_game_cases, stock_chips], [finished],
                            behavior=lambda x, y: [SimToken({'product_type': 5}, delay=1)],
                            name="produce_game", reward_function=lambda x, y: 1)
    ...
    def max_phone_production(pn, actions_dict):
        ...
        ordered_phone_cases = len(HeuristicSolver.get_place_tokens_by_type(place_id = 'ordered', desired_values = {'product_type': 0}, pn= pn))
        ...
        if 'produce_phone' in actions_dict.keys():
            assignable = HeuristicSolver.get_action_available_assignments(action='produce_phone', actions_dict=actions_dict)
        ...
        if 'order' in actions_dict.keys():
            ...
            if total_phone_cases >= total_chips: # order chips if we have more phone cases than chips
                ...
                ret_val = HeuristicSolver.get_attribute_based_assignments(action="order", attribute_dict={'product_type': 1}, actions_dict=actions_dict)
                return {'order': ret_val[0]}
            else:
                #for i in assignable:
                #    if i[0][1].value['product_type'] == 0:
                #        return {'order': i}
                ret_val = HeuristicSolver.get_attribute_based_assignments(action="order", attribute_dict={'product_type': 0}, actions_dict=actions_dict)
                return {'order': ret_val[0]}
    ...
    if test:
        ...
        #res = agency.testi
        #supply_chain.set_solver(solver_test)
        res_p = supply_chain.testing_run(solver_test, reporter=reporter)
