...
if __name__ == "__main__":
    ...
    run_name = "2025-05-29-19-05-03_run"
    ...
    arrival.put({'task_type': 0, 'case_id': 0}) #TODO: add a check in put to see if the the keys of the tokens put into places correspond with its var_attributes
    ...
    agency.add_event([busy2], [employee1, completed2], complete2, name='done2')

    def is_same_case_id(e1, e2):
        """
        This function checks if the case_id of two events is the same.
        :param e1: the first event
        :param e2: the second event
        :return: True if the case_id is the same, False otherwise
        """
        return e1['case_id'] == e2['case_id']

    agency.add_event([completed1, completed2], [], behavior=lambda x,y: [], name='doneFinal', reward_function=lambda x,y: 1, guard=is_same_case_id)
