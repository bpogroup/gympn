...
if __name__ == "__main__":
    ...
    run_name = '2025-05-29-19-37-01_run'
    ...
    waiting1 = agency.add_var("waiting1", var_attributes=['task_type'])#, categorical_values={'code_eployee': [0, 2, 1]})
    ...
    #waiting2.put({'task_type': 0})

    # Define resources.
    employee1 = agency.add_var("employee1", var_attributes=['code_employee'])#, categorical_values={'code_employee': [0, 2, 1]})
    ...
    #employee2 = agency.add_var("employee2", var_attributes=['code_employee'])#, categorical_values={'code_employee': [0, 2, 1]})
    ...
    def complete1(b):
        ...
        """
        return [SimToken(b[-1]), SimToken({'task_type': 1})]

    agency.add_event([busy1], [employee1, waiting2], complete1, name='done1')

    agency.add_action([waiting2, employee1], [busy2], behavior=start, name="start2")

    def start2(c, r):
        """
        Resource 0 takes 2 time unit to finish tasks of type 0, and 1 time units to finish tasks of type 1.
        Resource 1 takes 1 time units to finish tasks of type 0, and 2 time unit to finish tasks of type 1.
        Resource 2 takes 3 time units to finish any task.
        :param c: the task
        :param r: the resource
        :return: a list of SimTokens representing the task and the resource that were assigned to them
    ...
    def complete2(b):
        ...
        if b[0]['task_type'] == 0 and b[1]['code_employee'] == 0 or b[0]['task_type'] == 1 and b[1][
            'code_employee'] == 1:
            # if the task is of type 0 and the resource is of type 0 or if the task is of type 1 and the resource is of type 1
            # then the task is completed by the resource
            return [SimToken(b[-1]), None, SimToken(b[0])]
        else:
            # otherwise, the task needs to be redone
            return [SimToken(b[-1]), SimToken(b[0]), None]


    agency.add_event([busy2], [employee1, waiting1, completed], complete2, name='done2')

    agency.add_event([completed], [], behavior=lambda x: [], name='complete_case', reward_function=lambda x: 1)
