...
\item \textbf{An ablation ladder in which computing the estimand \emph{exactly}
does not pay} (Section~\ref{sec:exp}). \cdag{} computes the causal
estimand by construction and ranks second-worst of nine, beaten decisively by three biased
variants; the one variant that fails catastrophically is the one whose error is
\emph{unbounded}. What separates working from failing estimators here is
boundedness, not exactness---a distinction we can support, in contrast to
several mechanisms we tested and rejected
(Appendix~\ref{app:falsified}).
...
\citet{eventdriven2026} control a semiconductor fab---an event-driven system
much like ours, at industrial scale---and begin from an observation we share
almost exactly: because an action initiates a temporally extended event that
overlaps other events, ``the next decision point may correspond to a different
event and thus be uninformative for credit assignment.'' We record this
agreement explicitly, because it is independent evidence that the defect our
method targets is real and is felt in practice.

Their response to it differs from ours in the variable it groups on. They
define transitions over the temporal extent of the action-induced event rather
than to the next arbitrary decision---an SMDP-style retiming, as they note---and
then add an \emph{event-group} temporal-difference layer: all events falling in
a sampled time segment $I_h=[t,t+h]$ form a group $G$, and training matches the
group's aggregated TD discrepancy to a single system-level reward earned over
that interval, $\mathcal{L}(\theta)=\bigl(|G|^{-1}\sum_{e\in G}\delta_e(\theta)
-r^g(I_h)\bigr)^2$. A weighted variant replaces the mean with normalized
\emph{duration} shares $\omega_e \propto \Delta_e$ over the events overlapping a
sampled event's interval. For policy optimization they set GAE aside as
ill-suited to very long delays and use a group-relative surrogate
$\tilde{A}_i \triangleq \delta_i - r^g$, which subtracts the shared broadcast
reward from the critic-implied return in the manner of a group baseline.

So their layer does redistribute a delayed global reward across decisions, and
it would be wrong to call it only a retiming. But the group is a set of events
that \emph{co-occur in time}, and the weights are shares of elapsed duration.
Ours is the set of decisions a reward causally \emph{descends from}, and the
weights are shares of token flow. The two coincide only by accident: two lots
processed in the same 360-minute window are grouped together there whatever
their provenance, and two decisions on one wafer's route are separated there
once they fall in different windows, while our partition does the opposite in
both cases. The consequences follow from that difference. Their aggregation
needs a system-level reward signal defined over an interval to aggregate
\emph{toward}; ours needs only the rewards the episode already pays, each
assigned to a single owner, which is what makes it a return decomposition
rather than a reward-matching objective. Theirs applies to any discrete-event
simulator that reports event start and finish times; ours additionally requires
that the simulator record token identity, which is a stronger demand that A-E PN
execution happens to satisfy for free. And where they identify the theoretical
status of their objective as open---``whether and when it admits, approximates,
or preserves a Bellman-consistent fixed point''---the corresponding question for
our estimator is what Section~\ref{sec:theory} answers, though for a weaker
property: a bound on the bias relative to the closure estimand rather than a
...
\begin{equation}
  ...
  \label{eq:star}
\end{equation}
It is realized by \cdag{} \emph{by construction} rather than to within a
measured tolerance: at $\lambda=1$ with $V\equiv0$ its bootstrap term vanishes
and what the recursion computes is \eqref{eq:star} itself. We verify the
implementation matches the definition---the two agree to $0$ units in the last
place over $191$ decisions---but this is a check on code, not an estimate of a
quantity. Because ownership is a partition
...
\subsection{Computing the estimand exactly does not pay}
\label{sec:exactness}

\cdag{} computes \eqref{eq:star} by construction: it reproduces textbook GAE on
a chain to machine precision, and it counts a re-convergent diamond's reward
once rather than twice. It is the unbiased member of the family. It ranks
second-worst of nine.

The structural reason is visible in Table~\ref{tab:envs}. On a single-component
environment the descendant closure is most of the remaining episode
($S=0.474$, and $0.712$ on a related environment), so the exact estimand
approaches the unscoped return and exactness buys no scoping at all. Exactness
is a property of the target, and on a one-blob environment that target barely
varies with the action.

\paragraph{The weighted variants under-recover the estimand, and the shortfall
compounds with causal depth.} Every path-weighted variant places total weight
$c_d(j)\le1$ on a descendant's owned reward, so it recovers only a fraction of
\eqref{eq:star}. Measured at $\lambda=1$, $V\equiv0$, $\beta=0$ over five random
rollouts, the mean recovered fraction is $0.58$--$0.63$ on the single-component
environment and $0.89$--$0.91$ at four copies, against \cdag's $1$ by
construction, and it decays monotonically with causal depth---for \cflow{} on
the single-component environment, from $1.00$ at depth $1$ to $0.49$ at depth
$9$.

We report this as a property of the estimators rather than as an explanation of
the ordering, because on its own it cannot be one. Advantages are standardized
per batch before each policy update (Appendix~\ref{app:repro}), so a
\emph{uniform} shrinkage of all credit is invisible to the gradient by
construction; only the dispersion of $c_d$ can act, and that dispersion does not
order the variants either---on the single-component environment \cflowraw{} has
the \emph{lowest} coefficient of variation of the four ($0.42$ against
$0.49$--$0.64$) and is the arm that collapses. What the measurement does
establish is the fan-in dilution that motivates \cdag{} in the first place, and
that the dilution is systematic in depth rather than incidental.

\paragraph{What we could not explain.} The ordering of the four working arms is
not accounted for by any mechanism we tested. Six candidates were probed and all
six failed, two of them \emph{inverted}---per-state ranking fidelity against a
common-random-number counterfactual put \cdag{} first ($\rho=+0.681$, top-1
agreement $83.3\%$) against \cflow's $+0.435$ and $33.3\%$, and credit
signal-to-noise ratio likewise favoured the losing arm.
Appendix~\ref{app:falsified} reports each probe with the measurement that
rejected it. The only variable that separates working from failing arms without
overlap is final policy entropy ($0.27$--$0.56$ against $0.13$--$0.15$), and
whether that is cause or symptom is untested; the decisive experiment is an
entropy-bonus intervention, which we have not run. We also note that all six
probes were static and used a random policy, which may be the wrong regime for
the question. We therefore state the ordering as an open question. What the
ladder does support---and all it needs to support---is the negative claim in
Section~\ref{sec:normneeded}: exactness is not what separates the working
estimators from the failing one, and boundedness is.
...
\paragraph{The restricted role of the $N{=}2$ configuration.}
We use it as the low-$K$ end of this curve and for nothing else. It cannot
discriminate between estimators: at the converged budget the spread across all
seven arms there is $0.169$ with nothing significant, and at the shorter budget
the same configuration produced a spread of $0.576$ that \emph{was} significant,
in the wrong direction---the artifact that motivated the budget audit at the
head of this section. A configuration that reverses its verdict between budgets
and separates nothing at the longer one has too little headroom and too much
seed variance to rank methods. It earns its place here only as a point on the
$K$ axis, and we draw no conclusion from the ordering of arms within it.
...
Exactness is not the design objective; bounded error is. The precedent is GAE
itself, whose $\lambda<1$ settings are biased and universally preferred over the
unbiased $\lambda=1$ case. Our unbiased member performs worse than three biased
ones, and the one variant that fails catastrophically is the one whose error is
\emph{unbounded}. We put this negatively on purpose: it says what does
\emph{not} determine the outcome. Which of the bounded variants to prefer is not
something our evidence settles, and Section~\ref{sec:exactness} records the six
mechanisms we tested and rejected rather than offering a seventh we cannot
support.
...
\paragraph{What the method requires, and what it does not.}
The scope boundary is not the problem domain but the reward structure. Nothing
in the estimator is specific to task assignment: it needs only that rewards
attach to discrete, token-consuming firings whose provenance the simulator
records, and that enough of the reward mass sit in distinct causal components
for $K$ to exceed one. Admission control and network revenue management satisfy
the first condition directly, since revenue is earned at an acceptance, which is
a firing that consumes a request token. Multi-item inventory control does not
satisfy it straightforwardly: holding cost accrues per period rather than at a
firing, so there is no consuming transition for the reward to attach to, and the
cost would have to be discretized onto firings before ownership is even
well defined. We have not run a second domain, so this states what the
construction requires rather than where it has been shown to work.
...
Section~\ref{sec:exactness} reports that computing the causal estimand exactly
does not pay, and offers only a partial structural account of why the remaining
variants order as they do. That claim would be easy to dress up with a plausible mechanism. We
