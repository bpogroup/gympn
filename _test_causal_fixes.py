"""Quick test: verify the causal RL fixes work (no crash, reasonable output)."""
import numpy as np
import torch
import sys
...
def create_two_stage_env(causal_rl=False):
    # Token-flow postpone is required by the LRQ credit scheme whenever
    # postpone actions are enabled (postpone must be a lineage member).
    agency = GymProblem(allow_postpone=True, causal_rl=causal_rl,
                        causal_postpone_tokenflow=causal_rl)
...
args = {
    'episodes': 20, 'epochs': 20, 'batch_size': 32,
    'max_episode_length': None,
    'policy_lr': 3e-4, 'value_lr': 1e-3,
    'gam': 0.99, 'lam': 0.95, 'eps': 0.2,
    'vf_coeff': 0.5, 'ent_bonus': 0.01,
    'policy_kld_limit': 0.2,
    'causal_rl': True, 'algorithm': 'ppo-clip',
    'verbose': 1, 'use_gpu': False, 'agent_seed': 42,
    'use_wandb': False, 'open_tensorboard': False,
    'test_in_train': False, 'save_freq': 999999,
}

print("=" * 60)
print("Testing Causal RL (credits as direct advantages) on Two-Stage")
print("Fixes: no discount, no GAE, credit-V(s) advantages, normalized")
print("=" * 60)
...
try:
    ...
    if "Credits length" in str(e):
        print(f"\nLENGTH MISMATCH BUG DETECTED: {e}", file=sys.stderr)
        sys.exit(1)
    raise
...
if h and "mean_returns" in h:
    ...
    if len(post_peak) > 3 and np.mean(post_peak[-3:]) < 0.5 * best:
        print("WARN: Possible collapse after peak")
    else:
        print("OK: No catastrophic collapse")
else:
